...
type CharData = { char: string; romaji: string; color: string } | null;

// 50音表を行（段）ごとに管理することで、プレースホルダーのズレを防ぐ
const HIRAGANA_ROWS: CharData[][] = [
  [
...
export default function HiraganaGrid() {
  return (
    <div className="grid grid-cols-5 gap-4 md:gap-6 p-4 max-w-4xl mx-auto items-center justify-items-center">
      {HIRAGANA_ROWS.flatMap((row, rowIndex) =>
        row.map((item, colIndex) =>
          item ? (
            ...
            <div key={`empty-${rowIndex}-${colIndex}`} className="w-24 h-24 md:w-32 md:h-32" />
          )
        )
      )}
    </div>
